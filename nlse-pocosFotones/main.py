...
for i in range (len(Pot)):
    ...
    # Generate Lorentzian in frequency domain and apply IFTopt to get time-domain pump
    g = 0.001
    lorentzian_freq = np.sqrt(Pot[i]) * (1 / (np.pi *g*(1 + (sim.freq/g)**2)))
    pump_lorenziana = ttb.IFTopt(lorentzian_freq)  # Lorentzian pump in time domain
    
    # plt.figure(1)
    # plt.plot(sim.freq,(ttb.TFopt(pump_lorenziana)))
    plt.figure(1)
    #pump_cont = np.sqrt(Pot[i])*np.ones(puntos)
    pulso_1 = 0*0.1*np.sqrt(Pot[i])*np.ones(puntos)*np.exp(1j*2*np.pi*freqFWM*sim.tiempo)
    ruido = 0.5*np.sqrt(Pot[i])*np.random.normal(0,0.1,puntos)
    ...
    #plt.figure()
    espectro_in = np.abs(ttb.TFopt(pulso_0))**2
    dbmin = 10 * np.log10(espectro_in + 1e-12)  # Convertir a dBm, evitar log(0)
    lambdas = const.c / ((sim.freq * 1e12) + (const.c / (lambda0 * 1e-9))) * 1e9  # nm
    dbmout = 10*np.log10(np.abs(AW[-1])**2 + 1e-12)  # Convertir a dBm, evitar log(0)
    ...
    plt.subplot(212)
    """
    plt.plot(lambdas, dbmin, label = "Potencia de entrada = "+str(10*np.log10(Pot[i])+30)+" dBm")

    plt.title("Espectro de entrada", fontsize=16)
    plt.xlabel("Longitud de onda [nm]", fontsize=16)
    plt.ylabel("Potencia [dBm]", fontsize=16)
    plt.xlim(1529,1531)
    ...
    plt.plot(lambdas,dbmout, label=f"Potencia = {10*np.log10(Pot[i])+30} dBm")
...
plt.legend()
plt.savefig("nlseSPMsimulada.png")
plt.show()
