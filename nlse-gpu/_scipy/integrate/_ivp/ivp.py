...
def prepare_events(events):
    ...
    max_events = cp.empty(len(events))
...
def handle_events(sol, events, active_events, event_count, max_events,
    ...
    if cp.any(event_count[active_events] >= max_events[active_events]):
        if t > t_old:
        ...
        roots = roots[order]
        t = cp.nonzero(event_count[active_events]
...
def solve_ivp(fun, t_span, y0, method='RK45', t_eval=None, dense_output=False,
    ...
    **options
    ...
    if events is not None:
        ...
        event_count = cp.zeros(len(events))
        if args is not None:
