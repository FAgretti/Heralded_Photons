...
def SolveNLS(sim: Sim, fib: Fibra, pulso_0, z_locs=None, pbar = True):
    ...
    fs  = sim.fs
    sampnum = len(pulso_0)
    espectro_0 = ttb.TFoptGPU(pulso_0)
    
    #Calculamos el operador lineal
    if fib.betas:
        D_w = 0
        for i in range(len(fib.betas)):
            D_w = D_w + 1j*fib.betas[i]/cp.math.factorial(i+2) * (2*cp.pi*sim.freq)**(i+2)
        D_w = cp.array(D_w)

    #f_B = partial(dBdz, D = D_w, w = 2*cp.pi*sim.freq, gamma = fib.gamma, TR = fib.TR)
    f_B = lambda z, B: dBdz(z,B,D_w,2*cp.pi*sim.freq,fib.gamma)
    ...
    if pbar:  # Por si queremos la barra de progreso
        with tqdm(total=fib.L, unit="m") as pbar:
            ...
            def dBdz_with_progress(z, B):
                ...
                return dBdz(z, B, D = D_w, w = 2*cp.pi*sim.freq, gamma = fib.gamma)
            ...
            if z_locs:  # Si le pasamos un valor de z_locs: El array de salida tendra z_locs elementos
                t_eval = cp.linspace(0, fib.L, z_locs)
    ...
    for j in range( len(zlocs) ):
        A_w[j,:] = A_w[j,:] * cp.exp(D_w * zlocs[j])
        # print("z = ", zlocs[j])
    A_t = cp.array([ttb.IFToptGPU(a_w) for a_w in A_w], dtype=complex)
