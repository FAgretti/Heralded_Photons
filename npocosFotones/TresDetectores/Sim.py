...
"""
Defino los parámetros de simulación y la fibra
Fibra:
    L: Longitud de la fibra
    gamma: Coeficiente de no linealidad
    gamma1: Coeficiente de no linealidad de orden superior
    alpha: Atenuación
    lambda0: Longitud de onda central
    betas: Coeficientes de dispersión

Sim:
    puntos: Número de puntos sobre el cual tomar el tiempo
    Tmax: Tiempo máximo de simulación
    paso_t: Paso de tiempo
    tiempo: Vector de tiempo
    dW: Paso de frecuencia
    freq: Vector de frecuencia
    fs: Frecuencia de muestreo

Bin:
    N: numero de fotones
    lambda[N]: Longitud de onda de cada fotón
"""
...
class Sim:
    def __init__(self, puntos, Tmax):
        ...
        self.dW     = np.pi/Tmax
        self.freq   = np.fftshift( np.pi * np.arange(-puntos/2,puntos/2) / Tmax )/(2*np.pi)
