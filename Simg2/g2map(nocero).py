...
for k in range(numbins): 
    if np.random.rand()<0.4: photons[k]=2
det_input = np.array([np.random.binomial(i, 0.5) for i in photons])
g2fuente = np.mean(det_input*(photons-det_input))/np.mean(photons*0.5)**2
print(f"g2 fuente = {g2fuente}")
windows = np.linspace(1,100,100,dtype=int)   
dark = np.linspace(0,2,100)
g2 = np.zeros(100) 
for j in range(100):
    # window = int(i)
    window = 1
...
plt.plot(dark,g2,'r.--', markersize=10, linewidth=2, label='Simulación numérica')
x = np.linspace(1,100,100)
NSR = 2*dark/np.mean(photons)
noise_factor = (1+NSR)**2
g2med = (1-(1/noise_factor)+(g2fuente/noise_factor))

plt.plot(dark,g2med,'g--', linewidth=2, label='Aproximación analítica')
plt.grid()
plt.title(r'$g^{(2)}(0)$ vs. corriente de oscuridad para una fuente con $g^{(2)}(0)$ = 0.4', fontsize=20) #xx-small, x-small, small, medium, large, x-large, xx-large, larger, smaller, None
plt.xlabel('Corriente de oscuridad (fotones promedio por bin)', fontsize=18)
